...
class testViews(TestCase):
     ...
     def test_difficult_level(self):
         ...
         # self.assertEqual(None, views.get_difficulty_level(type('',(object,),{"GET": {}})()))
     ...
     def test_recipe_list_partition_ingredients(self, mock_get_filtered_recipes, mock_render_to_string, mock_get, mock_get_page):
     ...
     @mock.patch('django.template.loader.render_to_string')
     @mock.patch('search4recipes.models.Recipe.get_filtered_recipes')
     def test_recipe_list_ingredient_doesnt_exist(self, mock_get_filtered_recipes, mock_render_to_string, mock_get, mock_home):
         get_list = {"ingredient-list1" : "marchew1", "isPositive1" : 'on',
                            "ingredient-list2" : "marchew2", "isPositive2" : 'off',
                            "ingredient-list3" : "marchew3", "isPositive3" : 'asd',
                            "ingredient-list4" : "marchew4", "isPositive4" : '1',
     ...
     def test_update_mark_good_value(self, mock_get):
         ...
         self.assertEquals(recipe.rate, 0);

         params["mark"] = 3;
         views.update_mark(r)
         self.assertEquals(recipe.amount_of_rates, 2);
         self.assertEquals(recipe.rate, 3/2);

         params["mark"] = 5;
         views.update_mark(r)
         self.assertEquals(recipe.amount_of_rates, 3);
         self.assertEquals(recipe.rate, 8/3);

         params["mark"] = 10; #Nie wiem czy powinna metoda akceptowac liczby spoza zakresu gwiazdek
         views.update_mark(r)
         self.assertEquals(recipe.amount_of_rates, 4);
         self.assertEquals(recipe.rate, 18/4);
     ...
     def test_update_mark_divide_by_0(self, mock_get):
         ...
         mock_get.return_value = recipe;
         try:                   #Nie wiem czy powinienm to tak sprawdzac, bo to chyba metoda powinna sobie obsluzyc wyjatek
            views.update_mark(r)
